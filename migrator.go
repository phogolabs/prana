...
func (m *Migrator) Setup() error {
	...
	fmt.Fprintln(buffer, "CREATE SCHEMA IF NOT EXISTS system;")
	fmt.Fprintln(buffer)
	fmt.Fprintln(buffer, "CREATE TABLE system.migrations (")
	fmt.Fprintln(buffer, " id         TEXT      NOT NULL PRIMARY KEY,")
	fmt.Fprintln(buffer, " updated_at TIMESTAMP NOT NULL DEFAULT now(),")
	fmt.Fprintln(buffer, " created_at TIMESTAMP NOT NULL DEFAULT now(),")
	fmt.Fprintln(buffer, ");")
	fmt.Fprintln(buffer)
